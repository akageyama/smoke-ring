...
module field_m
  ...
  type, public :: field__fluid_t
    ...
    ! 表現するか、の違いである。
  contains
    procedure :: boundary_condition => field__fluid_boundary_condition
...
contains
  ...
  function operator_curl( a )
    ...
    call boundary_condition( operator_curl )
  ...
  function operator_div( a )
    ...
    call boundary_condition( operator_div )
      ! 境界の格子点は境界条件ルーチンで設定する
...
!   flux_sq = operator_dot_product(a.flux,a.flux)
    ...
    operator_energyintegral                          &
         = 0.5_DR * sum(    flux_sq(1:NXPP,1:NYPP,1:NZPP)  &
  ...
  function operator_laplacian_scalar( a )
    ...
    call boundary_condition( operator_laplacian_scalar )
      ! 境界の格子点は境界条件ルーチンで設定する
  ...
  function operator_laplacian_vector( a )
    ...
    call boundary_condition( operator_laplacian_vector )
      ! 境界の格子点は境界条件ルーチンで設定する
  ...
  function operator_scalarintegral( a )
    ...
    dvol = (grid%delta%x)*(grid%delta%y)*(grid%delta%z)
         ...
         ! 一様であることを仮定している。つまりdx, dy, dzは空間位置に
         ! 依存せず一定である。
    operator_scalarintegral = sum( a(2:NX_GLOBAL-1,  &
                                     2:NY_GLOBAL-1,  &
                                     2:NZ_GLOBAL-1) ) * dvol
      ! ここで配列演算の添字が1からNXではなく2からNX-1などに
      ! 限定されていることに注意。これは体積積分の範囲を計算領域の
  ...
  subroutine field__fluid_boundary_condition( fluid )
    class(field__fluid_t), intent(inout) :: fluid
    ...
    call mpiut__exchange( Parallel%comm,             &
                          ...
                          fluid%flux%z )
    call mpiut__barrier( Parallel%comm )
  end subroutine field__fluid_boundary_condition
