...
!           grid__initialize
  ...
  type grid__pos_t
    !! 格子点位置を収める構造体
    !! posはpositionの意味
    real(DR), dimension(0:NXPP1) :: x !! x座標
    real(DR), dimension(0:NYPP1) :: y !! y座標
    real(DR), dimension(0:NZPP1) :: z !! z座標
  end type grid__pos_t

  type grid_pos_by_global_ijk_t
    real(DR), dimension(NX_GLOBAL) :: x
    real(DR), dimension(NY_GLOBAL) :: y
    real(DR), dimension(NZ_GLOBAL) :: z
  end type grid_pos_by_global_ijk_t

  type grid_global_ijk_t  !! global (gi,gj,gk) from local (li,lj,lk)
    integer, dimension(0:NXPP1) :: gi
    integer, dimension(0:NYPP1) :: gj
    integer, dimension(0:NZPP1) :: gk
  end type grid_global_ijk_t

  type grid_local_ijk_from_global_ijk_t  !! The opposite of above.
    integer, dimension(NX_GLOBAL) :: li
    integer, dimension(NY_GLOBAL) :: lj
    integer, dimension(NZ_GLOBAL) :: lk
