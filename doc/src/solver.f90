...
!               t_start         t_end
    ...
    call ut__assert( T_START < T0 .and. T0 < T1 .and. T1 < T_END,  &
                    "solver_m(121): Time inconsistent.")
...
!!<
  ...
  subroutine solver__diagnosis( nloop, time, fluid )
    ...
    if ( maxval(abs(fluid%flux%x)) > ANOMALOUSLY_LARGE ) then
      !! 質量フラックスのx成分が異常に大きい。
      call ut__message( "solver_m(528): Massflux_x overflow." )
      Job%karte = "overflow" 
      return
    ...
    if ( maxval(abs(fluid%flux%y)) > ANOMALOUSLY_LARGE ) then
      call ut__message( "solver_m(535): Massflux_y overflow." )
      Job%karte = "overflow"
      return
        !! これ以上計算しても無駄（すぐに終了処理に入るべし）
    end if
    ...
    if ( maxval(abs(fluid%flux%z)) > ANOMALOUSLY_LARGE ) then
      !! 質量フラックスのz成分が異常に大きい。
      call ut__message( "solver_m(543): Massflux_z overflow." )
      Job%karte = "overflow"
      return
        !! これ以上計算しても無駄（すぐに終了処理に入るべし）
    end if
    ...
    if ( maxval(fluid%density) > ANOMALOUSLY_LARGE ) then
      !! 質量が異常に大きい
      call ut__message( "solver_m(551): Density overflow." )
      Job%karte = "overflow"
      return
        !! これ以上計算しても無駄（すぐに終了処理に入るべし）
    end if
    ...
    if ( maxval(fluid%pressure) > ANOMALOUSLY_LARGE ) then
      !! 圧力が異常に大きい
      call ut__message( "solver_m(559): Pressure overflow." )
      Job%karte = "overflow"
      return
        !! これ以上計算しても無駄（すぐに終了処理に入るべし）
    end if
    ...
    if ( minval(fluid%pressure) < 0.0_DR ) then
      !! 圧力が負になってしまっている
      call ut__message(" solver_m(567): Negative pressure." )
    ...
    if ( minval(fluid%density) < 0.0_DR ) then
      !! 密度が負になってしまっている
      call ut__message(" solver_m(575): Negative density." )
      Job%karte = "negative anormaly"
      return
        !! これ以上計算しても無駄（すぐに終了処理に入るべし）
    end if
